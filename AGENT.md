...
3. **Actionable alerts.** Every notification is a small, human-readable plain-text message (event name, account, region, principal, source IP, plus event-specific fields), formatted via `InputTransformer`. No JSON, no emoji, no raw CloudTrail blobs. CloudWatch alarm SNS actions are forbidden for detections that need event context: CloudWatch cannot transform the payload, so the email is a threshold-crossed stub. Use EventBridge. Metric filters remain valid for *dashboard* telemetry.
...
4. **`InputTransformer`.** Every rule MUST use the project's plain-text key/value `InputTemplate` (no JSON, no emoji). The output is a clean multi-line message: a `"[ASSK] Security alert: <EventName>"` header, a blank line, then aligned `"Label: <Var>"` lines, with `Event ID` last. Each line is wrapped in double quotes so EventBridge strips the quotes and joins the lines with newlines (the documented way to emit multi-line plain text). Keep the eight base fields (Event, Account, Region, Time, Source IP, Principal, Identity, Event ID) and append any event-specific fields between Identity and Event ID. Look at `EventRuleConfigChanges` or `EventRuleSecurityGroupChanges` for the canonical pattern. Raw CloudTrail blobs in alerts are not acceptable. AccessDenied (and similar "needs triage" detections) MAY insert a static `Why` / `Next` block between the header and the field list, and SHOULD put `Error` / `Message` before `Event ID`. Do not add Why/Next to high-signal success detections whose event name is already the rationale.
5. **Target.** The stack's existing `CtAlertingTopic` SNS topic. Do not create per-rule topics.
6. **Do not alert from CloudWatch alarms** for CloudTrail-derived detections. `AlarmAccessDenied` was removed for this reason: the SNS payload cannot include principal, API, IP, or error. Keep metric filters only when the dashboard needs the counter (see `CloudWatchLogsMetricsFilterAccessDenied`). The remaining alarms (console login failures, IMDSv1) are threshold-1 success/failure counters, not context-needed burst detections — do not add more of them.
7. **AccessDenied rules** (`EventRuleAccessDenied` in both templates) MUST stay `State: ENABLED` (write-only management events) with `readOnly: false` and `errorCode` matchers. Never switch them to `ENABLED_WITH_ALL_CLOUDTRAIL_MANAGEMENT_EVENTS` — that reintroduces Describe/Get/List inbox noise. Keep the AccessDenied `InputTemplate` short: two Why/Next lines, then facts. Do not include `userAgent` (AWS CLI metadata is a wall of noise).
8. **Mutating dedicated rules are success-only.** Any CloudTrail API rule that is not AccessDenied and is not a high-signal *read* (`GetCallerIdentity`, `GetPasswordData`, `BatchGetSecretValue`, `GetFileDownloadUrls`, IAM enumeration) MUST include `errorCode: [{ exists: false }]` so a denial does not look like a successful change and does not duplicate the AccessDenied email.
