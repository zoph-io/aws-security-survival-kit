...
7. **AccessDenied rules** (`EventRuleAccessDenied` in both templates) MUST stay `State: ENABLED` (write-only management events) with `readOnly: false` and `errorCode` matchers. Never switch them to `ENABLED_WITH_ALL_CLOUDTRAIL_MANAGEMENT_EVENTS` — that reintroduces Describe/Get/List inbox noise.
